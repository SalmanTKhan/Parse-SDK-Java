...
class OfflineStore {
    ...
    /* package */ OfflineStore() {
        this(new OfflineSQLiteOpenHelper());
    }
    ...
    public OfflineStore(Context context) {
        this.helper = new OfflineSQLiteOpenHelper();
    }
    ...
    private <T extends ParseObject> Task<List<T>> findAsync(
        ...
        if (pin == null) {
            ...
            queryTask = uuidTask.onSuccessTask(new Continuation<String, Task<ResultSet>>() {
                @Override
                public Task<ResultSet> then(Task<String> task) {
        ...
        return queryTask.onSuccessTask(new Continuation<ResultSet, Task<Void>>() {
            @Override
            public Task<Void> then(Task<ResultSet> task) {
                ResultSet cursor = task.getResult();
    ...
    /* package for OfflineQueryLogic */ <T extends ParseObject> Task<T> fetchLocallyAsync(
        ...
        if (objectId == null) {
            ...
            if (uuidTask == null) {
                ...
                jsonStringTask = uuidTask.onSuccessTask(new Continuation<String, Task<ResultSet>>() {
                    @Override
                    public Task<ResultSet> then(Task<String> task) {
                ...
                }).onSuccess(new Continuation<ResultSet, String>() {
                    @Override
                    public String then(Task<ResultSet> task) {
                        ResultSet cursor = task.getResult();
                        try {
            ...
            jsonStringTask =
                    db.queryAsync(OfflineSQLiteOpenHelper.TABLE_OBJECTS, select, where, args).onSuccess(
                            new Continuation<ResultSet, String>() {
                                @Override
                                public String then(Task<ResultSet> task) throws Exception {
                                    ResultSet cursor = task.getResult();
                                    if (cursor.isAfterLast()) {
                                        /*
    ...
    private Task<Void> unpinAsync(final String key, final ParseSQLiteDatabase db) {
        ...
        return Task.forResult((Void) null).continueWithTask(new Continuation<Void, Task<ResultSet>>() {
            @Override
            public Task<ResultSet> then(Task<Void> task) {
        ...
        }).onSuccessTask(new Continuation<ResultSet, Task<Void>>() {
            @Override
            public Task<Void> then(Task<ResultSet> task) {
                // DELETE FROM Objects
    ...
    private Task<Void> deleteObjects(final List<String> uuids, final ParseSQLiteDatabase db) {
        ...
        String where = OfflineSQLiteOpenHelper.KEY_UUID + " IN (" + ParseTextUtils.join(",", placeholders) + ")";
        // dynamic args
    ...
    private Task<Void> deleteDataForObjectAsync(final ParseObject object, final ParseSQLiteDatabase db) {
        ...
        Task<Void> unpinTask = uuidTask.onSuccessTask(new Continuation<String, Task<ResultSet>>() {
            @Override
            public Task<ResultSet> then(Task<String> task) {
            ...
            public Task<Void> then(Task<ResultSet> task) {
                ...
                for (final String uuid : uuids) {
                    Task<Void> unpinTask = getPointerAsync(uuid, db).onSuccessTask(task1 -> {
                        ParsePin pin = (ParsePin) task1.getResult();
